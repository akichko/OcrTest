...
import os
import yaml
#from app_settings import endpoint, key
...
def analyze_read():
    if len(sys.argv) < 2:
        print("Usage: python sample_analyze_read.py <file_path_or_url>")
        return
    ...
    if file_input.startswith("http://") or file_input.startswith("https://"):
        ...
        poller = document_intelligence_client.begin_analyze_document(
            "prebuilt-read", AnalyzeDocumentRequest(url_source=file_input)
        )
    else:
        # ローカルファイルの場合
        ...
        with open(file_input, "rb") as f:
            poller = document_intelligence_client.begin_analyze_document(
                "prebuilt-read", f, content_type="application/pdf"
            )
